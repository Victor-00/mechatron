        ...
        if (req.method === 'GET' && (req.url === '/' || req.url === '/login.html')) {
            ...
            req.on('end', () => {
                try {
                    ...
                    // 2. Check if user has already logged in
                    if (user.logged === true) {
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        return res.end(JSON.stringify({ success: false, message: 'This team has already logged in.' }));
                    }

                    // 3. Grant access and update the teams.json file
                    user.logged = true; // Mark as logged in
                    fs.writeFileSync(teamsFilePath, JSON.stringify(teams, null, 2)); // Save the change to the file
